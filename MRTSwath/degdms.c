...
----------------------------------------*/
	if ( ( *dms > MAXDMS ) || ( *dms < MINDMS ) )
	    LOG_RETURN_ERROR("Invalid DMS coordinate value", "degdms",ERR_RESP);
    }
    else
    {
	*dms = *deg;
...
----------------------------------------*/
	...
	{
	    if ( *dms <= 0 )
		LOG_RETURN_ERROR("Invalid lat/long coordinate value", "degdms",
                    ERR_RESP);
	}
...
-----------------------------------------------------------------*/
	...
	if ( ( tempmin > MAXMIN ) || ( tempmin < MINMIN ) )
	    LOG_RETURN_ERROR("Invalid tempmin coordinate value", "degdms",
                ERR_RESP);
...
-----------------------------------------------------------------*/
	...
	if ( ( tempsec > MAXSEC ) || ( tempsec < MINSEC ) )
	    LOG_RETURN_ERROR("Invalid tempsec coordinate value", "degdms",
                ERR_RESP);
    }
